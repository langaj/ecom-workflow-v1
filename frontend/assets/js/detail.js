...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!id) {
    document.getElementById('detail-content').innerHTML = '<div class="empty-state"><p>缺少任务 ID</p></div>';
    return;
  }
...
async function loadDetail(id) {
  ...
  try {
    ...
    content.innerHTML = `<div class="empty-state"><p>加载失败: ${err.message}</p></div>`;
    createToast('加载详情失败: ' + err.message, 'error');
  }
}

function renderBasicInfo(data) {
  const section = createSection('基础信息');
  section.innerHTML = `
...
function renderProductInfo(data) {
  ...
  const section = createSection('商品信息');
  section.innerHTML = `
    <div class="detail-grid">
      <div class="detail-field">
...
function renderReferences(data) {
  const section = createSection('参考图');
  const product = data.product_json || {};
  const images = (product.referenceImages || []).filter(i => i.url);
  if (images.length === 0) {
    section.innerHTML = '<div style="color:var(--gray-400);font-size:0.85rem;">无参考图</div>';
    return;
  }
...
function renderAttachments(data) {
  const section = createSection('附件素材');
  const product = data.product_json || {};
...
function renderVariantSpecInfo(data) {
  ...
  const section = createSection('变体 & 规格配置');
  let html = '';

  if (variants.length > 0) {
    html += '<h4 style="margin:8px 0;font-size:0.85rem;color:var(--gray-600);">变体</h4>';
  ...
  if (specs.length > 0) {
    html += '<h4 style="margin:8px 0;font-size:0.85rem;color:var(--gray-600);">规格</h4>';
    html += specs.map(s => `
...
function renderSKUList(data) {
  ...
  const section = createSection('SKU 列表');
  section.innerHTML = `
...
function renderGeneratedImages(data) {
  ...
  const section = createSection('生成图片');
  let html = '';

  if (mainImages.length > 0) {
    html += '<h4 style="margin:8px 0;font-size:0.85rem;color:var(--gray-600);">主图</h4>';
    html += `<div class="image-grid">${mainImages.map(url => `
      <div class="image-grid-item"><img src="${url}" alt="主图"></div>
    `).join('')}</div>`;
  }
...
function renderExcel(data) {
  ...
  section.innerHTML = `<a href="${excelUrl}" target="_blank" class="btn btn-primary btn-sm">下载 Excel 文件</a>`;
}
...
function renderJobs(data) {
  ...
  const section = createSection('任务日志');
    ...
    html += `
      <div class="card" style="padding:16px;margin-bottom:12px;">
        ...
        <div style="font-size:0.8rem;color:var(--gray-500);">
          步骤: ${job.current_step || '-'} | 进度: ${progress}%
        </div>
        <div class="timeline" style="margin-top:8px;">
          ${jobStatusOrder.filter(s => s !== 'completed' && s !== 'failed').map((step, i) => {
            ...
            const labels = {
              planning: '规划�?, main_image: '生成主图', detail_image: '生成详情�?,
          ...
          ${job.status === 'failed' ? '<div class="timeline-item active" style="color:var(--red-600);">失败</div>' : ''}
        </div>
        ...
        ${job.result_json && Object.keys(job.result_json).length > 0 ? `
          <details style="margin-top:8px;">
            <summary style="font-size:0.8rem;color:var(--gray-500);cursor:pointer;">查看结果数据</summary>
